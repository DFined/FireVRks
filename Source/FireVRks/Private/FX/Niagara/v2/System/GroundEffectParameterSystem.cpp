...
void UGroundEffectParameterSystem::Initialize()
{
	if (!IsInit)
	{
...
void UGroundEffectParameterSystem::AddParameter(UAbstractFormalParameter* Parameter)
{
	Parameters.Add(Parameter->GetId(), Parameter);
}

TMap<FDFId, UAbstractFormalParameter*>* UGroundEffectParameterSystem::GetOuterParameters()
{
	return &Parameters;
}

void UGroundEffectParameterSystem::GetOuterParametersInOrder(TArray<UAbstractFormalParameter*>& Result)
{
	DFU::GetValues(*GetOuterParameters(), Result);
}

FString UGroundEffectParameterSystem::GetId()
{
	return "_Internal_.DEFAULT_GROUND_EFFECT";
}

FString UGroundEffectParameterSystem::GetName()
{
	return "Ground System";
}

UIcon* UGroundEffectParameterSystem::GetIcon()
{
	return UDFStatics::ICONS->DEFAULT_EFFECT_ICON;
}


void UGroundEffectParameterSystem::SpawnSystem(USystemSpawnData* Data)
{
	...
	auto Effect = UNiagaraFunctionLibrary::SpawnSystemAtLocation(Data->GetWorldObject(), UDFStatics::GROUND_SYSTEM, Location);
	...
	switch (RotationType->GetOrdinal())
	{
		...
		case 0: //FIXED
			{
				Normal = FVector(NORMAL_VECTOR->GetValue(Context));
				break;
			}
		case 1: //FROM_SHELL
			{
