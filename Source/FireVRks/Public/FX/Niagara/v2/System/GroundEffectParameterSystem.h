...
#include "GroundEffectParameterSystem.generated.h"

UCLASS(Blueprintable, BlueprintType)
class FIREVRKS_API UGroundEffectParameterSystem : public UEffectSystem
{
	GENERATED_BODY()
...
public:
	UGroundEffectParameterSystem() = default;

	UPROPERTY()
	UBlockFormalParameter* MAIN_SETTINGS_BLOCK = UParamUtil::Global<UBlockFormalParameter, bool>("Effect Settings", true, true);

	UPROPERTY()
	UCompoundableFormalParameter* VELOCITY = UCompoundableFormalParameter::New<UIntFormalParameter, int>(UDFStatics::ANCHOR, "Velocity", true, 3000);
	UPROPERTY()
	...
	UPROPERTY()
	UCompoundableFormalParameter* SPRITE_SIZE = UCompoundableFormalParameter::New<UIntFormalParameter, int>(UDFStatics::ANCHOR, "Sprite size", true, 90);
	...
	UPROPERTY()
	UIntFormalParameter* CONE_ANGLE = UParamUtil::Global<UIntFormalParameter, int>("Cone angle", true, 0);
	UPROPERTY()
	UIntFormalParameter* CONE_ANGLE_VARIANCE = UParamUtil::Global<UIntFormalParameter, int>("Cone angle variance", true, 0);
	UPROPERTY()
	...
	UPROPERTY()
	UFloatFormalParameter* CLUSTER_SPACING = UParamUtil::Global<UFloatFormalParameter, float>("Cluster spacing", true, 0.05f);
	...
	static UGroundEffectParameterSystem* Make()
	{
		auto Sys = NewObject<UGroundEffectParameterSystem>(UDFStatics::ANCHOR);
		Sys->Initialize();
		Sys->SetId(UDFStatics::GROUND_SYSTEM_ID);
		Sys->SetDisplayName("Default Ground Effect");
		return Sys;
	}
